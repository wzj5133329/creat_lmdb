...
data_root_dir=/home/asd/Project/NCC_WZJ/YOLO/train/NCC/  # Modify the path with your folder having Images and Labels directories.
...
length_imgs=`ls -l ${data_root_dir}/image|grep '^-'|wc -l`
length_labels=`ls -l ${data_root_dir}/xml|grep '^-'|wc -l`
...
if [ ${length_imgs} != ${length_labels} ]; then
    ...
    j=0
	for img in `ls ${data_root_dir}/image|sort -h`
	do
		img_list[${j}]=${img}
		((j++))
	done
	...
	k=0
	for label in `ls ${data_root_dir}/xml|sort -h`
	do
		label_list[${k}]=${label}
		((k++))
	done
